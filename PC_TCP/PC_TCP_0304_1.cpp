...
HANDLE ghMutex;  //Mutex to protect "actualAngle"
HANDLE Mutex99;  //thread99's Mutex, to protect "sockErr99_Cnt"(thread99 socket error counter)
int sockErr99_Cnt = 0;
...
double targetAngle[6] = { 0, 0, 90, -25, 0, 90 };	// 目標角度
...
void RobotDataRead()		// 讀寫機器手臂資料
{
	...
	while (retry)  // ??????????????????????????????
	{
		...
		if ((status > 0) && (FD_ISSET(destSocket, &SockSet) != 0))
		{
			...
			WaitForSingleObject(ghMutex, INFINITE);  //ghMutex Lock!
...
DWORD WINAPI threadFunction1(LPVOID lpParameter) // 141Hz for dealing real robot
{
	...
	while (loop) {
		...
		for (int i = 0; i<6; i++) {
			...
			else if (fabs(delta)>0.05)  //0.5>差值>0.05
...
{
	char rBuffer[128];  //9*6+5+10=69 (6 joints * '(-)000.0000', round off to the 4th decimal + ',-777.7777' )  /// 60 -> 256 !??
	rBuffer[0] = 'a';
	char sBuffer[128];
	int bytesRecv;
	...
	WaitForSingleObject(ghMutex, INFINITE);  //ghMutex Lock!
	sprintf_s(sBuffer, "%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,-777.7777", \
	...
	else if (rBuffer[0] != 'a')
	{
		...
		char *token;
		char *next_token;
		token = strtok_s(rBuffer, s, &next_token);  //get the first token
...
DWORD WINAPI threadFunction5(LPVOID lpParameter)
{
	...
	while (1)//一直等待直到client連線成功
	{
		...
		while(1){
			thread99 = CreateThread(NULL, 0, threadFunction99, NULL, 0, NULL);  //收+發 資料to Unity
			//GetExitCodeThread(thread99, &thread99_isExit); //不能這樣令, thread_isExit會被洗掉
			//改用以下 Global variable:
			if(sockErr99_Cnt > 10)	break;  //if thread99 error exceed threshold=3 => break inner loop
		}
	}
...
{
	HANDLE thread1, thread5;
	
	thread1 = CreateThread(NULL, 0, threadFunction1, NULL, 0, NULL);
	thread5 = CreateThread(NULL, 0, threadFunction5, NULL, 0, NULL);
	ghMutex = CreateMutex(NULL, FALSE, NULL);  //between thread1&5: to protect "actualAngle"
	Mutex99 = CreateMutex(NULL, FALSE, NULL);  //between thread99s: to protect "sockErr99_Cnt" (thread99 socket error counter)
