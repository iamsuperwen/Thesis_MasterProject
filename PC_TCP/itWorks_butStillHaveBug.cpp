...
double targetAngle[6] = { 0, 0, 90, 5, 0, 90 };		// 目標角度
...
int main(int argc, char *argv[])
{
	...
	while (loop) {
		...
		double actualAngle[6] =  {0, 0, 90, -25, 0, 90};		// robot 實際角度
		double commandAngle[6] = {0, 0, 0, 0, 0, 0};	        // 從actualAngle ~ targetAngle 之間的過渡(慢慢加)
		double targetAngle[6] = {30, 0, 90, -25, 0, 90};		// 目標角度
		*/
		//if (status != SOCKET_ERROR)
			RobotDataRead();

		for (int i = 0; i<6; i++) {
			//commandAngle[i] = targetAngle[i] - actualAngle[i];
			double delta = targetAngle[i] - actualAngle[i];
			if (fabs(delta)>0.5)
				commandAngle[i] += delta/fabs(delta) * 0.2;
			else  //差小於一個值 => 忽略 (以免機械手微調造成異常
				commandAngle[i] = 0;
		}
